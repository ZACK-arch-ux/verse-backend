...
// Ensure username column exists (for old databases)
pool.query(`
  ALTER TABLE users
  ADD COLUMN IF NOT EXISTS username VARCHAR(100);
`)
.then(() => console.log("Username column ready"))
.catch(err => console.error("Username column error:", err));
...
app.get("/posts", async (req, res) => {
  try {
    const result = await pool.query(`
      SELECT posts.id, posts.content, posts.created_at, users.email, users.username
      FROM posts
